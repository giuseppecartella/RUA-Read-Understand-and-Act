...
from vision_transformer.timm.models.layers.drop import DropPath
from vision_transformer.timm.models.layers.mlp import Mlp
from vision_transformer.timm.models.layers.patch_embed import PatchEmbed
from vision_transformer.timm.models.layers.weight_init import lecun_normal_, trunc_normal_
from vision_transformer.timm.models.registry import register_model
...
class VisionTransformer(nn.Module):
    ...
    def _init_vit_weights(self,  module, name = '', head_bias = 0., jax_impl = False):
...
def _load_weights(model, checkpoint_path, prefix = ''):
    ...
    if hasattr(model.patch_embed, 'backbone'):
        ...
        stem.conv.weight.copy_(adapt_input_conv(stem.conv.weight.shape[1], _n2p(w['{}conv_root/kernel'.format(prefix)])))
        stem.norm.weight.copy_(_n2p(w['{}gn_root/scale'.format(prefix)]))
        stem.norm.bias.copy_(_n2p(w['{}gn_root/bias'.format(prefix)]))
        if not stem_only:
            for i, stage in enumerate(backbone.stages):
                for j, block in enumerate(stage.blocks):
                    bp = '{}block{}/unit{}/'.format(prefix, i+1,j+1)
                    for r in range(3):
                        getattr(block, 'conv{r + 1}'.format(r+1)).weight.copy_(_n2p(w['{}conv{}/kernel'.format(bp,r+1)]))
                        getattr(block, 'norm{r + 1}'.format(r+1)).weight.copy_(_n2p(w['{}gn{}/scale'.format(bp,r+1)]))
                        getattr(block, 'norm{r + 1}'.format(r+1)).bias.copy_(_n2p(w['{}gn{}}/bias'.format(bp,r+1)]))
                    if block.downsample is not None:
                        block.downsample.conv.weight.copy_(_n2p(w['{}conv_proj/kernel'.format(bp)]))
                        block.downsample.norm.weight.copy_(_n2p(w['{}gn_proj/scale'.format(bp)]))
                        block.downsample.norm.bias.copy_(_n2p(w['{}gn_proj/bias'.format(bp)]))
        embed_conv_w = _n2p(w['{}embedding/kernel'.format(prefix)])
    else:
        embed_conv_w = adapt_input_conv(
            model.patch_embed.proj.weight.shape[1], _n2p(w['{}embedding/kernel'.format(prefix)]))
    model.patch_embed.proj.weight.copy_(embed_conv_w)
    model.patch_embed.proj.bias.copy_(_n2p(w['{}embedding/bias'.format('prefix')]))
    model.cls_token.copy_(_n2p(w['{}cls'.format(prefix)], t=False))
    pos_embed_w = _n2p(w['{}Transformer/posembed_input/pos_embedding'.format(prefix)], t=False)
    if pos_embed_w.shape != model.pos_embed.shape:
        pos_embed_w = resize_pos_embed(  # resize pos embedding when different size from pretrained weights
            pos_embed_w, model.pos_embed, getattr(model, 'num_tokens', 1), model.patch_embed.grid_size)
    model.pos_embed.copy_(pos_embed_w)
    model.norm.weight.copy_(_n2p(w['{}Transformer/encoder_norm/scale'.format(prefix)]))
    model.norm.bias.copy_(_n2p(w['{}Transformer/encoder_norm/bias'.format(prefix)]))
    if isinstance(model.head, nn.Linear) and model.head.bias.shape[0] == w['{}head/bias'.format(prefix)].shape[-1]:
        model.head.weight.copy_(_n2p(w['{}head/kernel'.format(prefix)]))
        model.head.bias.copy_(_n2p(w['{}head/bias'.format(prefix)]))
    if isinstance(getattr(model.pre_logits, 'fc', None), nn.Linear) and '{}pre_logits/bias'.format(prefix) in w:
        model.pre_logits.fc.weight.copy_(_n2p(w['{}pre_logits/kernel'.format(prefix)]))
        model.pre_logits.fc.bias.copy_(_n2p(w['{}pre_logits/bias'.format(prefix)]))
    for i, block in enumerate(model.blocks.children()):
        block_prefix = '{}Transformer/encoderblock_{i}/'.format(prefix,i)
        mha_prefix = block_prefix + 'MultiHeadDotProductAttention_1/'
        block.norm1.weight.copy_(_n2p(w['{}LayerNorm_0/scale'.format(block_prefix)]))
        block.norm1.bias.copy_(_n2p(w['{}LayerNorm_0/bias'.format(block_prefix)]))
        block.attn.qkv.weight.copy_(torch.cat([
            _n2p(w['{}{}/kernel'.format(mha_prefix, n)], t=False).flatten(1).T for n in ('query', 'key', 'value')]))
        block.attn.qkv.bias.copy_(torch.cat([
            _n2p(w['{}{}/bias'.format(mha_prefix, n)], t=False).reshape(-1) for n in ('query', 'key', 'value')]))
        block.attn.proj.weight.copy_(_n2p(w['{}out/kernel'.format(mha_prefix)]).flatten(1))
        block.attn.proj.bias.copy_(_n2p(w['{}out/bias'.format(mha_prefix)]))
        for r in range(2):
            getattr(block.mlp, 'fc{}'.format(r+1)).weight.copy_(_n2p(w['{}MlpBlock_3/Dense_{}/kernel'.format(block_prefix, r)]))
            getattr(block.mlp, 'fc{}'.format(r+1)).bias.copy_(_n2p(w['{}MlpBlock_3/Dense_{}/bias'.format(block_prefix, r)]))
        block.norm2.weight.copy_(_n2p(w['{}LayerNorm_2/scale'.format(block_prefix)]))
        block.norm2.bias.copy_(_n2p(w['{}LayerNorm_2/bias'.format(block_prefix)]))
